...
## ЭТАП 1: Core UX (Foundation)
- [ ] 1.1 Форматирование текста (Markdown parser + renderer + toolbar)
  - [x] Добавить `entities` в `messagesTable` (JSONB)
  - [ ] Создать миграцию БД
  - [ ] Парсер Markdown на бэкенде (bold, italic, code, pre, spoiler, strikethrough, text_link)
  - [ ] Рендерер entities на фронтенде (MessageText компонент)
  - [ ] Floating toolbar при выделении текста
  - [ ] Горячие клавиши (Ctrl+B, Ctrl+I, Ctrl+Shift+M)
  - [ ] CSS стили для всех типов форматирования
- [ ] 1.2 Закреплённые чаты (pinned field + UI pin/unpin + sort)
- [ ] 1.3 Архив чатов (archive section + swipe + unread counter + auto-hide)
- [ ] 1.4 Сохранённые сообщения (auto-create self-chat on register + "Избранное" link)
- [ ] 1.5 Загрузка аватаров (crop to square + resize + upload API + display)
  - [ ] UI: загрузка фото профиля и чата
  - [ ] API: POST /users/avatar, POST /chats/:id/avatar
  - [ ] Кроп в квадрат + ресайз
  - [ ] Отображение аватаров в чат-листе и шапке
- [ ] 1.6 Экран настроек (full-screen modal: profile, notifications, privacy, security, data, language, about)

## ЭТАП 2: Messaging Enhancements
- [ ] 2.1 @упоминания (parse + highlight + clickable + notification)
- [ ] 2.2 Тихие сообщения (silent flag + no sound notification)
- [ ] 2.3 Автоудаление сообщений (timer per chat: 24h/7d/1mo + cron worker)
- [ ] 2.4 Спойлеры (||text|| → blurred blob + click to reveal)
