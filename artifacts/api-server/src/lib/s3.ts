...
// Public-facing client for presigned URLs (uses public endpoint)
...
export async function uploadFile(
  ...
  // Return a presigned URL (7 day expiry) using public endpoint
  const command = new GetObjectCommand({ Bucket: S3_BUCKET, Key: key });
  return getSignedUrl(publicS3, command, { expiresIn: 604800 });
}
...
export async function getPresignedUrl(key: string, expiresIn = 3600): Promise<string> {
  ...
  return getSignedUrl(publicS3, command, { expiresIn });
}
